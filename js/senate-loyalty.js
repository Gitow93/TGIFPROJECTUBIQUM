...
const leastLoyalty = senateDataMembers
  .sort((a, b) => b.votes_against_party_pct - a.votes_against_party_pct)
  .slice(0, 10);

const mostLoyalty = senateDataMembers
...
function createLeastLoyaltyTable(senateMembers) {
  ...
  for (let i = 0; i < senateMembers.length; i++) {
    ...
    tableRow.insertCell().innerHTML = senateMembers[i].total_votes;
    tableRow.insertCell().innerHTML = senateMembers[i].votes_against_party_pct;
...
function createMostLoyaltyTable(senateMembers) {
