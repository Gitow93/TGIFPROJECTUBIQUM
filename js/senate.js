...
function createTable(array) {
  //creamos una array para crear una tabla
  ...
  for (let i = 0; i < array.length; i++) {
    //hacemos un bucle para recorrer cada elemento de la array
...
function createDropdown(array) {
  // creamos una función para crear y poner las opciones del Dropdown
  ...
  states.forEach((key, index) => {
    // hacemos un 'forEach.' para que para cada uno de nuestros elementos 'KEY', haga lo siguiente:
...
function filterOptions(event) {
  //creamos una función donde pondremos el evento que deseamos filtrar (a los eventos siempre se les llama EVENT o EV!!)
  check = Array.from(checkboxes)
    .filter((element) => element.checked)
    .map((element) => element.value); // igualamos una constate state al 'value' de los checkboxes
  ...
  if (check.length === 0 && state === "All states") {
    ...
    // ahora hacemos una condición para cada caso que queramos
    event.forEach((element) => {
      if (check.length !== 0 && state === "All states") {
        // en esta primera opción aparecerá la selección 'todos los estados' pero se filtrará por el partido
        if (check.includes(element.party)) {
          // así pues le decimos que si nuestra array check incluye la propiedad party que queremos
        ...
        // en esta segunda opción hacemos lo mismo que en la primera pero con la propiedad de state
        if (element.state === state) {
          membersFiltered.push(element);
        }
      } else {
        if (check.includes(element.party) && element.state === state) {
          // en nuestra última condición le pedimos que si incluye la propiedad 'party' que queremos
...
dropdownMenu.addEventListener("change", () => {
  // en primer lugar el dropdown, donde nos tendrá que devolver la función de 'filterOptions'
  return filterOptions(members);
});
checkboxes.forEach(function (checkbox) {
  // en segundo lugar los checkboxes, donde utilizaremos el método 'forEach' para recorrer cada elemento de la array
  checkbox.addEventListener("change", () => {
    // en cada checkbox o elemento de la array añadimos un eventlistener
